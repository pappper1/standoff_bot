...
async def delete_game(call: types.CallbackQuery):
    if db.game_exists(call.from_user.id):
        ...
        if db.get_status(call.from_user.id) == 0:
            ...
            await call.answer("❗️ Вы не можете удалить эту игру, так как она сейчас начата!", show_alert= True) 
...
async def withdraw_amount(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        try:
            ...
            if 0 > amount < 50:
                await bot.send_message(message.from_user.id, "❗️ Минимальная сумма вывода - <b>50р</b>")
                ...
                await bot.send_message(message.from_user.id, f"‼️ На вашем балансе недостаточно средств!")
                ...
                await bot.send_message(message.from_user.id, f'💵 Сумма вывода: {amount} р\n\n🪪 Реквизиты: {info}\n\n🔰 Выберите действие ниже:', reply_markup= ikb.withdraw_confirmation())
...
async def withdraw_accept(call: types.CallbackQuery, state: FSMContext):
    ...
    for item in db.all_user_information(call.from_user.id):
        await bot.send_message(cfg.ADMIN, f'''🔰 Новая заявка на вывод!
...
💵Сумма вывода: {amount} р''')
    ...
    await bot.send_message(call.from_user.id, "✅ Заявка на вывод успешно создана!\n\n⌛️ Вывод осуществляется в течении 24 часов")
...
async def cash_in_amount(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        try:
            ...
            await bot.send_message(message.from_user.id, f"💵 Сумма пополнения: {amount} р\n\n🔰 Выберите действие ниже: ",
...
async def accept_cash_in(call: types.CallbackQuery, state: FSMContext):
    ...
    for item in db.all_user_information(call.from_user.id):
        await bot.send_message(cfg.ADMIN, f'''🔰 Новая заявка на пополнение!
...
💵Сумма пополнения: {amount} р''')
    ...
    await bot.send_message(call.from_user.id, "✅ Заявка на пополнение успешно создана!\n\n💈 Ожидайте, с вами свяжется администратор!")
