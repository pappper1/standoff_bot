    ...
    for item in db.get_all_active_games():
        ikb.add(
            InlineKeyboardButton(
                f"🎲 Ставка {item[5]} р",
                callback_data=games_cb.new(id=item[1], action="view"),
            )
        )
...
async def join_match(call: types.CallbackQuery, state: FSMContext):
    ...
    if db.get_user_status(call.from_user.id) == 1:
        await call.answer(
            "❌ Вы не можете присоединиться к матчу так как у вас открыт арбитраж!",
            show_alert=True,
        )

    elif db.game_exists(call.from_user.id):
        await call.answer(
            "❌ Вы не можете присоединиться к матчу, так как уже создали свой!",
            show_alert=True,
        )

    elif str(match_id) == str(call.from_user.id):
        await call.answer(
            "❌ Вы не можете присоединиться к своей же игре!", show_alert=True
        )
        ...
        try:
            ...
            ikb.add(
                InlineKeyboardButton(f"🏆 {call.from_user.id}", callback_data="joiner")
            )
...
""",
                ...
                await bot.send_message(
                    match_id,
                    f"""✅ К вашей игре присоединился {call.from_user.id}!
...
async def winner_creator(call: types.CallbackQuery, state: FSMContext):
    match_id = (await state.get_data())["match_index"]
    winner = call.data
    ...
    if str(call.from_user.id) == str(match_id):
        ...
        for item in db.get_results(match_id):
            if item[2] == "Выбор":
                ...
                await bot.send_message(
                    call.from_user.id,
                    "⚠️ Пришлите следующим сообщением все доказательства вашей победы(фото, видео): ",
                )
...
async def arbitrage_proofs(message: types.Message, state: FSMContext):
    ...
    ikb.add(
        InlineKeyboardButton(
            "♻️ Снять ограничения",
            callback_data=arbitrage_cb.new(id=f"index_{match_id}", action="end"),
        )
    )
    for item in db.get_game_information(match_id):
        await bot.send_message(
...
⬇️ Воспользуйтесь кнопкой ниже если арбитраж закрыт""",
    ...
    await bot.forward_message(settings.ADMIN, message.from_user.id, message.message_id)
    await bot.send_message(
        message.from_user.id, "📨 Ваши доказательства отправлены на рассмотрение!"
    )
    await state.finish()
