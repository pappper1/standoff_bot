...
async def send_spam(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        ...
        f"✅ Рассылка завершена!\n\nВсего отправлено сообщение: <b>{success_count}</b>\nЗаблокировало бота: <b>{bad_count}</b>",
    )
...
async def ban_user(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        message.from_user.id, f"✅ Пользователь {user_id} был успешно заблокирован!"
    )
    await asyncio.sleep(2)
    await start(message)
    await state.finish()
...
async def unban_user(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        message.from_user.id, f"✅ Пользователь {user_id} был успешно разблокирован!"
    )
    await asyncio.sleep(2)
    await start(message)
    await state.finish()
...
async def add_balance_user(message: types.Message, state: FSMContext):
    ...
    await state.update_data(user_balance=user)
    await bot.send_message(
        message.from_user.id,
        f"✏️ Введите сумму, которую хотите добавить пользователю <b>{user}</b>: ",
    )
...
async def add_balance_amount(message: types.Message, state: FSMContext):
    user = (await state.get_data())["user_balance"]
    if message.text.isdigit():
        amount = int(message.text)
        await state.update_data(amount_balance=amount)
        await bot.send_message(
            message.from_user.id,
            f"""🔰 Добавление баланса:
        
🆔 ID: {user}
💵 Сумма: {amount} р

🔆 Выберите действие ниже:""",
            reply_markup=ikb.add_balance_confirmation(),
        )
...
async def add_balance_confirm(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(
            call.from_user.id,
            f"✅ {amount} р успешно зачислено на баланс пользователя {user}",
        )
